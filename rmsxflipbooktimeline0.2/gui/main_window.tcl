...
namespace eval ::RMSXFlipbookTimeline::GUI {
    ...
    proc run_native_analysis {} {
        ...
        if {$native_topology eq "" || ![file exists $native_topology]} {
            set_status "Choose a topology PDB first."
            return
        }
        if {$native_trajectory eq "" || ![file exists $native_trajectory]} {
            set_status "Choose a trajectory DCD first."
            return
        }
        ...
        if {[catch {
            ...
            if {[string trim $native_slice_size] ne ""} {
                set slice_size_value [validate_int "Native slice size" $native_slice_size 1]
            } else {
                set slices_value [validate_int "Native slices" $native_slices 1]
            }
            set start_value [validate_int "Native start frame" $native_start 0]
            set end_value [validate_int "Native end frame" $native_end -1]
            set time_step_value [validate_double "Native time step" $native_time_step 0.0]
        } err]} {
            set_status $err
            return
        }

        set native_metric [string trim $timeline_metric]
        if {$native_metric eq ""} {
            set native_metric "RMSX"
            set timeline_metric $native_metric
        }
        ::RMSXFlipbookTimeline::state_set native_metric $native_metric
        ::RMSXFlipbookTimeline::state_set timeline_metric $timeline_metric

        if {![metric_is_native_flipbook $native_metric]} {
            run_native_timeline_metric $native_metric $start_value $end_value
            return
        }

        if {$native_output eq ""} {
            set_status "Choose an output folder for native RMSX."
            return
        }

        set metric_key [string tolower [string trim $native_metric]]
        set shift_metric [expr {$metric_key in {shift shiftmap shift-map {shift map}}}]
        set lddt_metric [expr {$metric_key in {lddt {lddt map} lddt-map {1-lddt} {1 - lddt}}}]
        if {$shift_metric} {
            set metric_label "shift-map"
            ...
            set metric_label "lDDT map"
            set single_proc ::RMSXFlipbookTimeline::run_native_lddt_map
            set all_proc ::RMSXFlipbookTimeline::run_native_all_chain_lddt_map
        } else {
            set metric_label "RMSX"
            set single_proc ::RMSXFlipbookTimeline::run_native_analysis
            set all_proc ::RMSXFlipbookTimeline::run_native_all_chain_analysis
        }

        if {!$shift_metric && !$lddt_metric} {
            if {$slice_size_value ne "" && $slice_size_value < 2} {
                set_status "RMSX needs at least 2 frames per slice; use Slice Size 2 or larger."
                return
            }
            if {$slice_size_value eq "" && $end_value >= $start_value} {
                set requested_frames [expr {$end_value - $start_value + 1}]
                set planned_slice_size [expr {int($requested_frames / $slices_value)}]
                if {$planned_slice_size < 2} {
                    set max_slices [expr {int($requested_frames / 2)}]
                    set_status [format {RMSX needs at least 2 frames per slice; %d slices across frames %d-%d gives slice_size=%d and all-zero B-factors. Use %d slices or fewer, or increase the frame range.} \
                        $slices_value $start_value $end_value $planned_slice_size $max_slices]
                    return
                }
            }
        }

        set_status "Running VMD-native $metric_label...\n$native_topology\n$native_trajectory"
        update idletasks

        if {$slice_size_value ne ""} {
        ...
        if {$chain_mode in {all * auto}} {
            set native_command [list \
                $all_proc \
                $native_topology \
                $native_trajectory \
                $native_output]
            lappend native_command {*}$slicing_args \
                ...
                -progress_callback ::RMSXFlipbookTimeline::GUI::native_progress \
                -verbose 0
        } else {
            set native_command [list \
                $single_proc \
        ...
        if {[catch {uplevel #0 $native_command} native_result]} {
            set_status "Native $metric_label failed:\n$native_result"
            return
        }

        set folder [dict get $native_result output_dir]
        set_status "Native $metric_label complete. Loading generated flipbook...\n$folder"
        update idletasks

        if {[catch {
            ::RMSXFlipbookTimeline::load_folder $folder {*}$options write_manifest 1
        } load_result]} {
            set_status "Native $metric_label completed, but load failed:\n$load_result"
            return
        }

        set lines [list "Native $metric_label complete."]
        if {[dict exists $native_result csv]} {
            lappend lines "CSV: [dict get $native_result csv]"
        }
        if {[dict exists $native_result csv_paths]} {
            lappend lines "CSVs: [llength [dict get $native_result csv_paths]] chain outputs"
        }
        if {[dict exists $native_result rmsd_csv] && [file exists [dict get $native_result rmsd_csv]]} {
            lappend lines "RMSD: [dict get $native_result rmsd_csv]"
        }
        if {[dict exists $native_result rmsf_csv] && [file exists [dict get $native_result rmsf_csv]]} {
            lappend lines "RMSF: [dict get $native_result rmsf_csv]"
        }
        if {[dict exists $native_result summary_csv] && [file exists [dict get $native_result summary_csv]]} {
            lappend lines "Summary: [dict get $native_result summary_csv]"
        }
        if {[dict exists $native_result chains]} {
            lappend lines "Chains: [join [dict get $native_result chains] {, }]"
        }
        if {[dict exists $native_result combined_slice_count]} {
            lappend lines "Combined slices: [dict get $native_result combined_slice_count]"
        }
        if {[dict exists $native_result combined_mask_file] && [dict get $native_result combined_mask_file] ne ""} {
            lappend lines "Combined mask: [dict get $native_result combined_mask_file]"
        }
        if {[dict exists $native_result log_transform] && [dict get $native_result log_transform]} {
            lappend lines "Log transform: on"
        }
        if {[dict exists $native_result masked_residue_count] && [dict get $native_result masked_residue_count] > 0} {
            lappend lines "Masked residues: [dict get $native_result masked_residue_count]"
        }
        if {[dict exists $native_result deleted_previous_slices] && [dict get $native_result deleted_previous_slices] > 0} {
            lappend lines "Removed old slice PDBs: [dict get $native_result deleted_previous_slices]"
        }
        lappend lines [format_result $load_result]
        if {[catch {enable_mouse_rotation 1} mouse_result]} {
            lappend lines "Mouse rotation not enabled: $mouse_result"
        } else {
            lappend lines "Mouse rotation enabled: [format_mouse_status $mouse_result]"
