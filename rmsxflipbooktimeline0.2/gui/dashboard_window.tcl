...
namespace eval ::RMSXFlipbookTimeline::Dashboard {
    ...
    variable dashboard_default_height 860
    ...
    proc metric_metadata {} {
        set records [list \
            ...
            [dict create key lDDT label lDDT group Primary route native panel native description "1-lDDT flipbook slices"] \
            ...
            [dict create key secondary_structure label secondary_structure group Primary route timeline panel structure description "DSSP-style structure categories"] \
            [dict create key native_contacts label native_contacts group Primary route timeline panel contacts description "Fraction of native contacts retained"] \
            [dict create key displacement_velocity label displacement_velocity group Motion route timeline panel timeline description "Frame-to-frame displacement"] \
            ...
            [dict create key delta_phi label delta_phi group Structure route timeline panel angles description "Delta phi from first frame"] \
            [dict create key psi label psi group Structure route timeline panel angles description "Backbone psi angle"] \
            [dict create key delta_psi label delta_psi group Structure route timeline panel angles description "Delta psi from first frame"] \
            [dict create key hbonds label hbonds group Contacts route timeline panel contacts description "Hydrogen-bond event matrix"] \
            [dict create key salt_bridges label salt_bridges group Contacts route timeline panel contacts description "Salt-bridge event matrix"] \
            [dict create key inter_selection_contacts label inter_selection_contacts group Contacts route timeline panel contacts description "Contacts between selections"]]
    ...
    proc native_metric_values {} {
        return [metric_values_for_route native]
    }
    ...
    proc ensure_timeline_metric {} {
        ...
        if {[metric_route $timeline_metric] eq "native" || [lsearch -exact [timeline_metric_values] $timeline_metric] < 0} {
            set timeline_metric displacement
        }
    ...
    proc set_status {message {append 0}} {
        variable status_widget
        if {$status_widget ne "" && [info commands winfo] ne "" && [winfo exists $status_widget]} {
    ...
    proc refresh_output_display_indicator {} {
        ...
        set current [normalize_dashboard_path $native_output]
        set displayed [normalize_dashboard_path $displayed_flipbook_path]
        set color "#257a3d"
        if {$displayed eq ""} {
            set output_display_var ""
        } elseif {[output_paths_match $current $displayed]} {
            set output_display_var "\u2713"
            set color "#257a3d"
        } else {
            set output_display_var "!"
        ...
        set widget [easy_child files].native_output_label.status
        if {[info commands winfo] ne "" && [winfo exists $widget]} {
            catch {$widget configure -foreground $color}
        }
    }
    ...
    proc dashboard_molid {} {
        ...
        if {$source_type eq "new_analysis" \
            && [file exists $native_topology] \
            && [file exists $native_trajectory] \
            && [info commands mol] ne ""} {
            return [file_backed_timeline_molid]
        }

        set requested [string trim $timeline_molid]
        if {[info commands molinfo] ne ""} {
            if {$requested eq "" || $requested eq "top"} {
                if {[catch {molinfo top} top_molid] == 0} {
                    return $top_molid
                }
            }
            if {[string is integer -strict $requested]} {
                return [expr {int($requested)}]
            }
        }

        if {[info commands mol] ne "" && $native_topology ne "" && [file exists $native_topology]} {
            set molid [mol new $native_topology waitfor all]
            if {$native_trajectory ne "" && [file exists $native_trajectory]} {
                mol addfile $native_trajectory waitfor all molid $molid
            }
            set timeline_molid $molid
            ::RMSXFlipbookTimeline::state_set timeline_molid $molid
            return $molid
        }
    ...
    proc draw_embedded_heatmap {dataset} {
        ...
        foreach record $records {
            ...
            $embedded_heatmap_canvas create rectangle \
                ...
                -tags [list embedded_pickable embedded_heat_cell $tag]
        ...
        ::RMSXFlipbookTimeline::TimelinePlot::draw_legend \
            ...
            $dataset
        $embedded_heatmap_canvas bind embedded_pickable <Button-1> {::RMSXFlipbookTimeline::Dashboard::embedded_heatmap_pick_current}
        $embedded_heatmap_canvas bind embedded_pickable <B1-Motion> {::RMSXFlipbookTimeline::Dashboard::embedded_heatmap_pick_current}
    ...
    proc native_plot_time_axis_args {} {
        ...
        if {[dict get $plan known]} {
            if {![catch {set step_ps [effective_time_step_ps_for_run]}] && [string trim $step_ps] ne ""} {
                lappend args time_step_ps $step_ps total_frames [dict get $plan used]
                if {[dict exists $plan first]} {
                    lappend args first_frame [dict get $plan first]
                } elseif {[string is integer -strict [string trim $native_start]]} {
                    lappend args first_frame [expr {int($native_start)}]
                }
            }
        }
        return $args
    ...
    proc draw_embedded_native_plot {folder csv_path metric} {
        ...
        if {$embedded_heatmap_canvas ne "" && [info commands winfo] ne "" && [winfo exists $embedded_heatmap_canvas]} {
            ...
            $embedded_heatmap_canvas bind pickable <Button-1> {::RMSXFlipbookTimeline::Dashboard::embedded_native_plot_pick_current}
    ...
    proc timeline_live_options {} {
        ...
        set opts [list selection $timeline_selection first_frame $first last_frame $last]
        lappend opts \
    ...
    proc run_native_metric {} {
        ...
        if {$chain_mode in {all * auto}} {
            set command [list $all_proc $native_topology $native_trajectory $native_output]
        } else {
            set command [list $single_proc $native_topology $native_trajectory $native_output -chain $native_chain]
        }
        ...
        if {[dict exists $result output_dir]} {
            ...
            set folder $native_output
        }
        if {[file isdirectory $folder]} {
            catch {::RMSXFlipbookTimeline::load_folder $folder palette [::RMSXFlipbookTimeline::state_get palette viridis]}
            focus_flipbook_scene
            ensure_default_mouse_rotation
            catch {render_native_heatmap $native_metric}
        }
    ...
    proc result_total_frame_count {result} {
        ...
        if {[dict exists $result total_frame_count]} {
            ...
            if {[string is integer -strict $total] && int($total) > 1} {
                return [expr {int($total)}]
            }
        }
    ...
    proc clear_scene {} {
        ...
        if {[catch {::RMSXFlipbookTimeline::reset 1} result]} {
            set_status "Clear failed:\n$result"
            return
        }
    ...
    proc close_window {} {
        variable top
        if {[catch {::RMSXFlipbookTimeline::cleanup_side_effects 0} result]} {
    ...
    proc load_existing_folder {} {
        ...
        if {$folder eq "" || ![file isdirectory $folder]} {
            set_status "Choose an existing RMSX folder first."
            return
        }
        if {[catch {::RMSXFlipbookTimeline::load_folder $folder palette [::RMSXFlipbookTimeline::state_get palette viridis]} result]} {
            set_status "Existing folder load failed:\n$result"
            return
        }
        focus_flipbook_scene
        ensure_default_mouse_rotation
        catch {render_native_heatmap}
        add_dataset_item "RMSX Folder" flipbook $folder
        set_status [format "Loaded existing RMSX folder: %s" $folder]
        refresh_dashboard
        return $result
    }
    ...
    proc render_native_heatmap {{metric ""}} {
        ...
        variable timeline_metric
        if {$folder eq "" || ![file isdirectory $folder]} {
            error "Choose an RMSX folder before opening the heatmap."
        }
        set matrix_metric [ensure_native_metric]
        if {$metric ne ""} {
            set matrix_metric $metric
        } elseif {[metric_route $timeline_metric] eq "native"} {
            set matrix_metric $timeline_metric
            set native_metric $timeline_metric
        }
        set plot_folder $folder
        set csv_path [native_metric_matrix_csv $folder $matrix_metric]
        if {$csv_path ne ""} {
            set plot_folder [file dirname $csv_path]
        }
        set result [draw_embedded_native_plot $plot_folder $csv_path $matrix_metric]
        set detail $folder
        if {$csv_path ne ""} {
            set detail $csv_path
        }
        return [dict create metric $matrix_metric detail $detail result $result]
    }
    ...
    proc current_native_folder {} {
        variable folder
        set selected [string trim $folder]
        if {$selected eq ""} {
            set selected [::RMSXFlipbookTimeline::state_get source_folder ""]
        }
        if {$selected eq ""} {
            error "Choose or load an RMSX flipbook folder first."
        }
        if {![file isdirectory $selected]} {
            error "RMSX flipbook folder does not exist: $selected"
        }
        set folder [file normalize $selected]
        return $folder
    }
    ...
    proc copy_user_field {} {
        variable timeline_user_field
        if {[catch {::RMSXFlipbookTimeline::copy_timeline_to_user $timeline_user_field} result]} {
            set_status "Copy to user field failed:\n$result"
            return
        }
        set_status "Copied matrix values to $timeline_user_field."
        return $result
    }
    ...
    proc export_current {kind} {
        ...
        set command ::RMSXFlipbookTimeline::write_current_timeline_tml
        if {$kind eq "svg"} {
            set command ::RMSXFlipbookTimeline::write_current_timeline_svg
        } elseif {$kind eq "png"} {
            set command ::RMSXFlipbookTimeline::write_current_timeline_png
        }
        if {[catch {$command $filename {*}[timeline_plot_options]} result]} {
            if {$kind eq "tml"} {
                if {[catch {$command $filename} result]} {
    ...
    proc refresh_slice_preview {} {
        ...
        if {![dict get $plan known]} {
            ...
            $canvas create text [expr {$width / 2}] [expr {$y0 + ($bar_h / 2.0)}] -text "Choose topology and trajectory to count frames" -anchor center -font {Helvetica 9}
            return
        }
        ...
        if {$frames <= 0 || $slices <= 0 || $slice_size <= 0} {
            ...
            $canvas create text [expr {$width / 2}] [expr {$y0 + ($bar_h / 2.0)}] -text "Requested slicing cannot produce slices" -anchor center -font {Helvetica 9}
            return
        }
    ...
    proc style_primary_action_button {button state} {
        ...
        catch {$button configure \
            -background "#cfe8ff" \
            -foreground "#062b5f" \
            -activebackground "#9dccff" \
            -activeforeground "#062b5f" \
            -disabledforeground "#062b5f" \
            -highlightbackground "#5aa7f2" \
            -highlightcolor "#5aa7f2" \
            -relief raised \
            -borderwidth 2}
    }
    ...
    proc set_action_button {button text command state} {
        ...
        if {[string match "*.run" $button]} {
            $button configure \
                -text $text \
                -command [list ::RMSXFlipbookTimeline::Dashboard::invoke_action $command] \
                -state normal
            style_primary_action_button $button normal
            return
        }
    ...
    proc refresh_source_controls {} {
        ...
        set native_ready [expr {[file exists $native_topology] && [file exists $native_trajectory] && [string trim $native_output] ne ""}]
        ...
        set source_title "1. Analysis Setup: choose topology, trajectory, and output folder."
        ...
        switch -- $source_type {
            existing_folder {
                ...
                set source_title "1. Existing Results: choose an RMSX folder, then inspect or pop out the heatmap."
            }
            default {
        ...
        set_action_button $actions.save "Save Image" ::RMSXFlipbookTimeline::Dashboard::save_flipbook_image $save_state
    ...
    proc refresh_slicing_mode_controls {} {
        ...
        if {$native_slicing_mode eq "slice_size"} {
            if {[winfo exists $settings.native_slice_size_label]} {
                grid $settings.native_slice_size_label -row 0 -column 3 -sticky e -padx {4 2} -pady 1
                grid $settings.native_slice_size_entry -row 0 -column 4 -sticky w -padx {0 4} -pady 1
            }
        } else {
            if {[winfo exists $settings.native_slices_label]} {
                grid $settings.native_slices_label -row 0 -column 3 -sticky e -padx {4 2} -pady 1
                grid $settings.native_slices_entry -row 0 -column 4 -sticky w -padx {0 4} -pady 1
            }
        }
    }
    ...
    proc build_file_row {parent row label variable_name browse_kind} {
        ...
        ttk::entry $parent.${variable_name}_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::$variable_name -style Path.TEntry
    ...
    proc build_source_selector {parent} {
        set row 0
        foreach {value label} {
            new_analysis "New PDB/DCD Analysis"
            existing_folder "Existing RMSX Folder"
        } {
            ttk::radiobutton $parent.source_$value \
                -text $label \
                -value $value \
                -variable ::RMSXFlipbookTimeline::Dashboard::source_type \
                -command ::RMSXFlipbookTimeline::Dashboard::refresh_dashboard
            grid $parent.source_$value -row 0 -column $row -sticky w -padx {0 14} -pady 2
            incr row
        }
    ...
    proc build_metric_controls {parent {include_advanced_toggle 0}} {
        ...
        ttk::combobox $parent.metric_combo \
            -textvariable ::RMSXFlipbookTimeline::Dashboard::native_metric \
            -values [native_metric_values] \
            -state readonly \
            -width 14
        ...
        ttk::combobox $parent.palette_combo \
            -textvariable ::RMSXFlipbookTimeline::Dashboard::palette \
            -values [palette_values] \
            -state readonly \
            -width 9
        bind $parent.palette_combo <<ComboboxSelected>> {::RMSXFlipbookTimeline::Dashboard::on_palette_changed}

        ttk::label $parent.summary_label -text "Meaning"
        ttk::label $parent.summary_text -text "" -wraplength 300
        ttk::label $parent.color_label -text "Color"
        ttk::label $parent.color_text -text "" -wraplength 440
        ...
        grid $parent.metric_label -row 0 -column 0 -sticky w -pady 3
        grid $parent.metric_combo -row 0 -column 1 -sticky w -padx {8 12} -pady 3
        grid $parent.palette_label -row 0 -column 2 -sticky w -pady 3
        grid $parent.palette_combo -row 0 -column 3 -sticky w -padx {8 14} -pady 3
        grid $parent.summary_label -row 0 -column 4 -sticky w -pady 3
        grid $parent.summary_text -row 0 -column 5 -sticky ew -padx {8 0} -pady 3
        if {$include_advanced_toggle} {
            grid $parent.advanced_check -row 0 -column 6 -sticky e -padx {14 0} -pady 3
        }
        grid $parent.color_label -row 1 -column 0 -sticky nw -pady 3
        grid $parent.color_text -row 1 -column 1 -columnspan [expr {$include_advanced_toggle ? 6 : 5}] -sticky ew -padx {8 0} -pady 3
        grid columnconfigure $parent 5 -weight 1
    }
    ...
    proc build_timeline_metric_controls {parent} {
        ...
        ttk::label $parent.metric_label -text "Metric"
        ttk::combobox $parent.metric_combo \
            -textvariable ::RMSXFlipbookTimeline::Dashboard::timeline_metric \
            -values [timeline_metric_values] \
            -state readonly \
            -width 24
        ...
        ttk::label $parent.group_label -text "Category"
        ttk::label $parent.group_text -text ""
        ttk::label $parent.route_label -text "Route"
        ttk::label $parent.route_text -text ""
        ttk::label $parent.summary_label -text "Meaning"
        ttk::label $parent.summary_text -text "" -wraplength 300
        ttk::label $parent.color_label -text "Color"
        ttk::label $parent.color_text -text "" -wraplength 560

        grid $parent.metric_label -row 0 -column 0 -sticky w -pady 3
        grid $parent.metric_combo -row 0 -column 1 -sticky ew -padx {8 12} -pady 3
        grid $parent.palette_label -row 0 -column 2 -sticky w -pady 3
        grid $parent.palette_combo -row 0 -column 3 -sticky w -padx {8 14} -pady 3
        grid $parent.summary_label -row 0 -column 4 -sticky w -pady 3
        grid $parent.summary_text -row 0 -column 5 -sticky ew -padx {8 0} -pady 3
        grid $parent.color_label -row 1 -column 0 -sticky nw -pady 3
        grid $parent.color_text -row 1 -column 1 -columnspan 5 -sticky ew -padx {8 0} -pady 3
        grid columnconfigure $parent 5 -weight 1
    }

    proc build_run_settings {parent} {
        ttk::label $parent.mode_label -text "By"
        ttk::radiobutton $parent.mode_slices \
            -text "Slices" \
            -value slices \
            -variable ::RMSXFlipbookTimeline::Dashboard::native_slicing_mode \
            -command ::RMSXFlipbookTimeline::Dashboard::refresh_dashboard
        ttk::radiobutton $parent.mode_size \
            -text "Size" \
            -value slice_size \
            -variable ::RMSXFlipbookTimeline::Dashboard::native_slicing_mode \
            -command ::RMSXFlipbookTimeline::Dashboard::refresh_dashboard
        ttk::label $parent.native_slices_label -text "Slices"
        ttk::entry $parent.native_slices_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_slices -width 3
        ttk::label $parent.native_slice_size_label -text "Size"
        ttk::entry $parent.native_slice_size_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_slice_size -width 4
        ttk::label $parent.native_total_frames_label -text "Total frames"
        ttk::entry $parent.native_total_frames_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_total_frames -width 6
        ttk::label $parent.native_total_time_ns_label -text "Time"
        ttk::entry $parent.native_total_time_ns_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_total_time_ns -width 4
        ttk::label $parent.native_total_time_ns_unit -text "ns"
        ttk::label $parent.native_chain_label -text "Chain"
        ttk::entry $parent.native_chain_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_chain -width 3
        ttk::label $parent.native_start_label -text "Frame"
        ttk::entry $parent.native_start_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_start -width 4
        ttk::label $parent.native_end_label -text "-"
        ttk::entry $parent.native_end_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_end -width 4
        ttk::label $parent.native_time_step_label -text "Step"
        ttk::entry $parent.native_time_step_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::native_time_step -width 5
        ttk::label $parent.native_time_step_unit -text "ps"

        grid $parent.mode_label -row 0 -column 0 -sticky w -padx {0 2} -pady 1
        grid $parent.mode_slices -row 0 -column 1 -sticky w -padx {0 0} -pady 1
        grid $parent.mode_size -row 0 -column 2 -sticky w -padx {0 0} -pady 1
        grid $parent.native_chain_label -row 0 -column 5 -sticky e -padx {0 2} -pady 1
        grid $parent.native_chain_entry -row 0 -column 6 -sticky w -padx {0 4} -pady 1
        grid $parent.native_start_label -row 0 -column 7 -sticky e -padx {0 2} -pady 1
        grid $parent.native_start_entry -row 0 -column 8 -sticky w -padx {0 2} -pady 1
        grid $parent.native_end_label -row 0 -column 9 -sticky e -padx {0 2} -pady 1
        grid $parent.native_end_entry -row 0 -column 10 -sticky w -padx {0 4} -pady 1
        grid $parent.native_time_step_label -row 0 -column 11 -sticky e -padx {0 2} -pady 1
        grid $parent.native_time_step_entry -row 0 -column 12 -sticky w -padx {0 2} -pady 1
        grid $parent.native_time_step_unit -row 0 -column 13 -sticky w -padx {0 4} -pady 1
        grid $parent.native_total_time_ns_label -row 0 -column 14 -sticky e -padx {0 2} -pady 1
        grid $parent.native_total_time_ns_entry -row 0 -column 15 -sticky w -padx {0 2} -pady 1
        grid $parent.native_total_time_ns_unit -row 0 -column 16 -sticky w -pady 1
        canvas $parent.preview -width 260 -height 18 -background white -highlightthickness 1 -highlightbackground "#9a9a9a"
        ttk::label $parent.preview_text -text "" -wraplength 500
        grid $parent.preview -row 1 -column 0 -columnspan 17 -sticky ew -pady {1 0}
        grid $parent.preview_text -row 2 -column 0 -columnspan 17 -sticky ew -pady {0 0}
        bind $parent.preview <Configure> {::RMSXFlipbookTimeline::Dashboard::refresh_slice_preview}

        foreach widget [list \
            $parent.native_slices_entry \
            $parent.native_slice_size_entry \
            $parent.native_total_frames_entry \
            $parent.native_total_time_ns_entry \
            $parent.native_chain_entry \
            $parent.native_start_entry \
            $parent.native_end_entry \
            $parent.native_time_step_entry] {
            bind $widget <KeyRelease> {after idle ::RMSXFlipbookTimeline::Dashboard::refresh_dashboard}
            bind $widget <FocusOut> {::RMSXFlipbookTimeline::Dashboard::refresh_dashboard}
        }

        grid columnconfigure $parent 17 -weight 1
        refresh_slicing_mode_controls
    }
    ...
    proc build_metric_option_panels {parent {include_advanced_toggle 1}} {
        ...
        ttk::button $advanced_panel.classic -text "Open Classic / Legacy Controls" -command ::RMSXFlipbookTimeline::Dashboard::open_classic
    ...
    proc build_actions {parent} {
        ...
        button $parent.run \
            -text "Run RMSX" \
            -command ::RMSXFlipbookTimeline::Dashboard::run_native_metric \
            ...
            -padx 6 \
            -pady 3
        style_primary_action_button $parent.run normal
        ttk::button $parent.save -text "Save Image" -command ::RMSXFlipbookTimeline::Dashboard::save_flipbook_image
        ttk::button $parent.popout -text "Heatmap ↗" -command ::RMSXFlipbookTimeline::Dashboard::open_full_plot_window
        ttk::button $parent.spacing_minus -text "Spacing −" -width 12 -command {::RMSXFlipbookTimeline::Dashboard::nudge_spacing -2.0}
        ttk::button $parent.spacing_plus -text "Spacing +" -width 12 -command {::RMSXFlipbookTimeline::Dashboard::nudge_spacing 2.0}
        ttk::button $parent.reset_view -text "Reset View" -command ::RMSXFlipbookTimeline::Dashboard::reset_view
        ttk::button $parent.clear -text "Clear" -command ::RMSXFlipbookTimeline::Dashboard::clear_scene
        grid $parent.run -row 0 -column 0 -sticky ew -padx {0 6}
        grid $parent.save -row 0 -column 1 -sticky ew -padx {0 6}
        grid $parent.popout -row 0 -column 2 -sticky ew -padx {0 6}
        grid $parent.spacing_minus -row 0 -column 3 -sticky ew -padx {0 6}
        grid $parent.spacing_plus -row 0 -column 4 -sticky ew -padx {0 6}
        grid $parent.reset_view -row 0 -column 5 -sticky ew -padx {0 6}
        grid $parent.clear -row 0 -column 6 -sticky ew
    }
    ...
    proc build_embedded_heatmap_panel {parent} {
        ...
        grid columnconfigure $parent 0 -weight 1 -uniform heatmap_footer
        grid columnconfigure $parent 1 -weight 1 -uniform heatmap_footer
        ...
        grid $parent.summary -row 1 -column 0 -sticky ew -pady {3 0}
        grid $parent.context -row 2 -column 0 -sticky w -pady {2 0}
        grid $parent.citation -row 1 -column 1 -rowspan 2 -sticky ne -padx {14 0} -pady {2 0}
        bind $parent.canvas <Configure> {::RMSXFlipbookTimeline::Dashboard::schedule_embedded_heatmap_resize_redraw %w}
        bind $parent.citation <Configure> {::RMSXFlipbookTimeline::Dashboard::resize_label_wrap %W %w 300 6}
        clear_embedded_heatmap
    }
    ...
    proc build_scrollable_easy_content {parent} {
        grid columnconfigure $parent 0 -weight 1
        grid rowconfigure $parent 0 -weight 1
        canvas $parent.scroll_canvas \
            -highlightthickness 0 \
            -borderwidth 0 \
            -background [ttk::style lookup TFrame -background]
        ttk::frame $parent.content -padding {0 0}
        ...
        bind $parent.scroll_canvas <MouseWheel> [list ::RMSXFlipbookTimeline::Dashboard::scroll_canvas_by_wheel $parent.scroll_canvas %D]
        bind $parent.scroll_canvas <Button-4> [list ::RMSXFlipbookTimeline::Dashboard::scroll_canvas_by_wheel $parent.scroll_canvas 120]
        bind $parent.scroll_canvas <Button-5> [list ::RMSXFlipbookTimeline::Dashboard::scroll_canvas_by_wheel $parent.scroll_canvas -120]
        set root [winfo toplevel $parent]
        bind $root <MouseWheel> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_wheel $parent.scroll_canvas %D]
        bind $root <Button-4> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_wheel $parent.scroll_canvas 120]
        bind $root <Button-5> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_wheel $parent.scroll_canvas -120]
        bind $root <Prior> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas Prior]
        bind $root <Next> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas Next]
        bind $root <Home> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas Home]
        bind $root <End> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas End]
        bind $root <Up> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas Up]
        bind $root <Down> [list ::RMSXFlipbookTimeline::Dashboard::scroll_easy_canvas_by_key $parent.scroll_canvas Down]
    ...
    proc build_easy_tab {parent} {
        ...
        set source [ttk::labelframe $content.source -text "1. Analysis Setup: choose topology, trajectory, and output folder." -padding 8]
        set files [ttk::labelframe $content.files -text "2. Input Files" -padding 8]
        set metric [ttk::labelframe $content.metric -text "3. Heatmap Metric" -padding 8]
        set settings [ttk::labelframe $content.settings -text "4. Slice Settings" -padding {6 5}]
    ...
    proc build_matrix_tab {parent} {
        grid columnconfigure $parent 0 -weight 1
        set metric [ttk::labelframe $parent.metric -text "1. Matrix Setup: choose a metric, molecule, and selection." -padding 6]
        set live [ttk::labelframe $parent.live -text "2. Live Matrix" -padding 6]
        set import [ttk::labelframe $parent.import -text "Saved Matrix Sources" -padding 6]
        set filter [ttk::labelframe $parent.filter -text "3. Scale / Filter" -padding 6]
        ...
        ttk::entry $live.molid_entry -textvariable ::RMSXFlipbookTimeline::Dashboard::timeline_molid -width 10
        ...
        button $live.plot \
            -text "Open Matrix" \
            -command ::RMSXFlipbookTimeline::Dashboard::run_live_metric \
            -font {Helvetica 11 bold} \
            -foreground "#062b5f" \
            -background "#cfe8ff" \
            -activeforeground "#062b5f" \
            ...
            -padx 6 \
            -pady 3
        style_primary_action_button $live.plot normal
        ...
        grid $live.molid_entry -row 0 -column 1 -sticky ew -padx {8 16} -pady 3
        grid $live.selection_label -row 0 -column 2 -sticky w -pady 3
        grid $live.selection_entry -row 0 -column 3 -sticky ew -padx {8 16} -pady 3
        grid $live.plot -row 0 -column 4 -sticky w -pady 3
        grid $live.columns_label -row 1 -column 0 -sticky w -pady 3
        grid $live.columns_frames -row 1 -column 1 -sticky w -padx {8 6} -pady 3
        grid $live.columns_slices -row 1 -column 2 -sticky w -padx {0 16} -pady 3
        grid $live.aggregation_label -row 1 -column 3 -sticky e -pady 3
        grid $live.aggregation_combo -row 1 -column 4 -sticky ew -padx {8 16} -pady 3
        grid $live.representative_label -row 2 -column 0 -sticky w -pady 3
        grid $live.representative_combo -row 2 -column 1 -sticky ew -padx {8 16} -pady 3
        ...
        foreach {col label var width} {
            0 "Scale" timeline_scale_mode 14
            2 "Filter Min" timeline_filter_min 8
            4 "Filter Max" timeline_filter_max 8
            6 "Min Frames" timeline_filter_frames 8
        } {
            ...
            grid $filter.${var}_label -row 0 -column $col -sticky w -padx {0 6} -pady 3
            grid $filter.${var}_entry -row 0 -column [expr {$col + 1}] -sticky ew -padx {0 12} -pady 3
        }
        ttk::button $filter.apply -text "Apply Filter" -command ::RMSXFlipbookTimeline::Dashboard::apply_filter
        grid $filter.apply -row 0 -column 8 -sticky ew -pady 3
    ...
    proc build_export_tab {parent} {
        ...
        set image_panel [ttk::labelframe $parent.image -text "Image Export" -padding 6]
        set user_panel [ttk::labelframe $parent.user -text "Copy Matrix to VMD User Field" -padding 6]
        set export_panel [ttk::labelframe $parent.files -text "Matrix Files" -padding 6]
        grid $image_panel -row 0 -column 0 -sticky ew -pady {0 6} -padx {0 6}
        grid $user_panel -row 0 -column 1 -sticky ew -pady {0 6}
        grid $export_panel -row 1 -column 0 -columnspan 2 -sticky ew

        grid columnconfigure $image_panel 0 -weight 1
        grid columnconfigure $image_panel 1 -weight 1
        ttk::button $image_panel.save -text "Save Image" -command ::RMSXFlipbookTimeline::Dashboard::save_flipbook_image
        ttk::checkbutton $image_panel.transparent_png \
            -text "Transparent PNG" \
            -variable ::RMSXFlipbookTimeline::Dashboard::render_transparent_png \
            -command ::RMSXFlipbookTimeline::Dashboard::persist_common_state
        grid $image_panel.save -row 0 -column 0 -sticky ew -padx {0 10} -pady 3
        grid $image_panel.transparent_png -row 0 -column 1 -sticky w -pady 3

        ttk::label $user_panel.field_label -text "User Field"
        ttk::combobox $user_panel.field_combo -textvariable ::RMSXFlipbookTimeline::Dashboard::timeline_user_field -values {user user2 user3 user4} -state readonly -width 10
        ttk::button $user_panel.copy -text "Copy" -command ::RMSXFlipbookTimeline::Dashboard::copy_user_field
        grid $user_panel.field_label -row 0 -column 0 -sticky w -pady 3
        grid $user_panel.field_combo -row 0 -column 1 -sticky ew -padx {8 10} -pady 3
        grid $user_panel.copy -row 0 -column 2 -sticky ew -pady 3
        grid columnconfigure $user_panel 1 -weight 1

        foreach col {0 1 2} {
            grid columnconfigure $export_panel $col -weight 1
        }
        ttk::button $export_panel.tml -text "Export TML" -command {::RMSXFlipbookTimeline::Dashboard::export_current tml}
        ttk::button $export_panel.svg -text "Export SVG" -command {::RMSXFlipbookTimeline::Dashboard::export_current svg}
        ttk::button $export_panel.png -text "Export PNG" -command {::RMSXFlipbookTimeline::Dashboard::export_current png}
        grid $export_panel.tml -row 0 -column 0 -sticky ew -padx {0 8}
        grid $export_panel.svg -row 0 -column 1 -sticky ew -padx {0 8}
        grid $export_panel.png -row 0 -column 2 -sticky ew
    }
    ...
    proc build_advanced_tab {parent} {
        ...
        grid $native_panel -row 1 -column 0 -sticky ew -pady {0 6} -padx {0 6}
        grid $note -row 1 -column 1 -sticky ew -pady {0 6}
        if {$experimental} {
            ...
            grid $proc_panel -row 2 -column 0 -sticky ew -padx {0 6}
            grid $cross_panel -row 2 -column 1 -sticky ew
        }
        ...
        ttk::label $note.text -text "Open the full dense control surface when you need every legacy option." -wraplength 300
        ttk::button $note.classic -text "Open Classic / Legacy Controls" -command ::RMSXFlipbookTimeline::Dashboard::open_classic
    ...
    proc configure_dashboard_styles {} {
        catch {ttk::style configure TButton -padding {4 2} -font {Helvetica 10}}
        catch {ttk::style configure TCheckbutton -padding {2 1} -font {Helvetica 10}}
        catch {ttk::style configure TRadiobutton -padding {2 1} -font {Helvetica 10}}
        catch {ttk::style configure Path.TEntry -font {Helvetica 9}}
        catch {ttk::style configure Citation.TLabel -font {Helvetica 6} -foreground "#6b7280"}
    }

    proc build {} {
        variable top
        variable dashboard_min_width
        variable dashboard_default_height
        variable status_widget

        package require Tk
        configure_dashboard_styles
        if {[winfo exists $top]} {
            wm title $top "Flipbook"
            wm deiconify $top
            raise $top
            return $top
        }
        ...
        toplevel $top
        wm title $top "Flipbook"
        wm geometry $top ${dashboard_min_width}x${dashboard_default_height}
        wm minsize $top $dashboard_min_width 740
        wm protocol $top WM_DELETE_WINDOW ::RMSXFlipbookTimeline::Dashboard::close_window
        grid columnconfigure $top 0 -weight 1
        grid rowconfigure $top 0 -weight 1

        set tabs [ttk::notebook $top.tabs]
        grid $tabs -row 0 -column 0 -sticky nsew -padx 8 -pady {8 8}
        foreach {name label} {
            easy "RMSX / Flipbook"
            matrix "Interactive Timeline"
            export "Export"
            advanced "Advanced"
            help "Help"
        } {
            ttk::frame $tabs.$name -padding 8
            $tabs add $tabs.$name -text $label
        }

        build_easy_tab $tabs.easy
        build_matrix_tab $tabs.matrix
        build_export_tab $tabs.export
        build_advanced_tab $tabs.advanced
        build_help_tab $tabs.help

        set status [text $top.status \
            -height 1 \
            -state disabled \
            -wrap word \
            -relief flat \
            ...
            -highlightthickness 0 \
            -font {Helvetica 10}]
        set status_widget $status
        grid $status -row 1 -column 0 -sticky ew -padx 8 -pady {0 4}
        set_status "Dashboard ready. Start in RMSX / Flipbook for heatmaps, or Interactive Timeline for frame-linked matrices."
        refresh_dashboard
        return $top
    }
