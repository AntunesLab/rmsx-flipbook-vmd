...
namespace eval ::RMSXFlipbookTimeline::TimelinePlot {
    ...
    proc draw_threshold_graph {dataset layout} {
        ...
        $canvas create text $left [expr {$y0 - 14}] -anchor w -font {Helvetica 9 bold} -fill "#333333" -text "Threshold count ($min_value to $max_value)"
    ...
    proc draw {dataset layout palette} {
        ...
        $canvas create text $left 24 -anchor w -font {Helvetica 14 bold} -fill "#222222" -text [dict get $dataset title]
        $canvas create text [expr {$left + $plot_w}] 24 -anchor e -font {Helvetica 10} -fill "#555555" -text [dict get $dataset value_label]
        ...
        for {set row 0} {$row < [llength $rows]} {incr row $row_step} {
            ...
            $canvas create text [expr {$left - 8}] $y -anchor e -font {Helvetica 8} -fill "#444444" -text [::RMSXFlipbookTimeline::Matrix::row_label [lindex $rows $row]]
        }
        ...
        for {set col 0} {$col < [llength $columns]} {incr col $col_step} {
            ...
            $canvas create text $x [expr {$heat_bottom + 8}] -anchor n -font {Helvetica 8} -fill "#444444" -text [::RMSXFlipbookTimeline::Matrix::column_label [lindex $columns $col]]
        }
    ...
    proc draw_category_legend {canvas x y w h categories label} {
        ...
        $canvas create text $x [expr {$y - 6}] -anchor sw -font {Helvetica 8 bold} -fill "#555555" -text $label
    ...
    proc draw_legend {canvas x y w h min_val max_val palette label {dataset {}}} {
        ...
        $canvas create text $x [expr {$y + $h + 5}] -anchor nw -font {Helvetica 8} -fill "#555555" -text [format "%.3g" $min_val]
        $canvas create text [expr {$x + $w}] [expr {$y + $h + 5}] -anchor ne -font {Helvetica 8} -fill "#555555" -text [format "%.3g" $max_val]
        $canvas create text [expr {$x + ($w / 2.0)}] [expr {$y + $h + 5}] -anchor n -font {Helvetica 8} -fill "#555555" -text $label
    }
    ...
    proc select_record {record} {
        ...
        if {[molid_is_loaded $molid]} {
            ...
            add_highlight_rep $molid [::RMSXFlipbookTimeline::Matrix::row_selection $row]
    ...
    proc pick_callback {name element op} {
        ...
        set sel [atomselect $molid "index $vmd_pick_atom"]
        try {
            if {[$sel num] == 0} {
                return
            }
            set resid [lindex [$sel get resid] 0]
            set chain [string trim [lindex [$sel get chain] 0]]
            set segid [string trim [lindex [$sel get segid] 0]]
        } finally {
            catch {$sel delete}
        }
        set row_index -1
        ...
        for {set i 0} {$i < [llength $rows]} {incr i} {
            ...
            if {![dict exists $row resid] || [string trim [dict get $row resid]] ne [string trim $resid]} {
                continue
            }
            set row_chain [expr {[dict exists $row chain] ? [string trim [dict get $row chain]] : ""}]
            set row_segid [expr {[dict exists $row segid] ? [string trim [dict get $row segid]] : ""}]
            if {$row_chain eq "" || $row_chain eq $chain || $row_chain eq $segid || $row_segid eq $segid} {
                set row_index $i
                break
            }
        }
