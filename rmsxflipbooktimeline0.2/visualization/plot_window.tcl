...
namespace eval ::RMSXFlipbookTimeline::PlotWindow {
    ...
    proc draw_line_chart {canvas x y w h points title color {point_radius 2.8} {label_position top}} {
        ...
        $canvas create text [expr {$x - 12}] $y -anchor e -font {Helvetica 9} -fill "#555555" -text [format_plot_number $y_max]
        $canvas create text [expr {$x - 12}] [expr {$y + $h}] -anchor e -font {Helvetica 9} -fill "#555555" -text [format_plot_number $y_min]
    }
    ...
    proc draw_summary_record_chart {canvas x y w h records title y_min y_max {color "#252525"}} {
        ...
        $canvas create text [expr {$x - 8}] $y -anchor e -font {Helvetica 8} -fill "#555555" -text [format_plot_number $y_max]
        $canvas create text [expr {$x - 8}] [expr {$y + $h}] -anchor e -font {Helvetica 8} -fill "#555555" -text [format_plot_number $y_min]
    ...
    proc draw_rmsf_side_chart {canvas layout rmsf_points} {
        ...
        foreach panel [dict get $layout panels] {
            ...
            $canvas create text $x [expr {$y + $h + 8}] -anchor nw -font {Helvetica 9} -fill "#555555" -text [format_plot_number $min_val]
            $canvas create text [expr {$x + $w}] [expr {$y + $h + 8}] -anchor ne -font {Helvetica 9} -fill "#555555" -text [format_plot_number $max_val]
        }
    }
    ...
    proc draw_legend {canvas x y w h min_val max_val palette metric_label} {
        ...
        $canvas create rectangle $x $y [expr {$x + $w}] [expr {$y + $h}] -outline "#444444"
        $canvas create text $x [expr {$y + $h + 5}] -anchor nw -font {Helvetica 8} -fill "#555555" -text [format_plot_number $min_val]
        $canvas create text [expr {$x + $w}] [expr {$y + $h + 5}] -anchor ne -font {Helvetica 8} -fill "#555555" -text [format_plot_number $max_val]
        $canvas create text [expr {$x + ($w / 2.0)}] [expr {$y + $h + 5}] -anchor n -font {Helvetica 8} -fill "#555555" -text $metric_label
    }
    ...
    proc select_record {record} {
        ...
        if {$slice_zero >= 0 && $slice_zero < [llength $molids]} {
            ...
            if {[record_kind $record] eq "cell"} {
                add_highlight_rep $molid [selection_for_record $record]
            }
            catch {mol top $molid}
        }
    ...
    proc record_for_structure_residue {slice_index resid chain segid} {
        ...
        foreach record $plot_records {
            ...
            if {[string trim [dict get $record resid]] ne $resid} {
                continue
            }
            if {[record_chain_matches $record $chain $segid]} {
                return $record
            }
            if {$fallback eq ""} {
                set fallback $record
    ...
    proc select_structure_atom {molid atom_index} {
        ...
        try {
            ...
            set segid [lindex [$sel get segid] 0]
        } finally {
            catch {$sel delete}
        }

        set slice_index [expr {$slice_zero + 1}]
        set record [record_for_structure_residue $slice_index $resid $chain $segid]
        if {$record eq ""} {
            error "No plot cell matches slice $slice_index residue $resid"
        }
        return [select_record $record]
    }
