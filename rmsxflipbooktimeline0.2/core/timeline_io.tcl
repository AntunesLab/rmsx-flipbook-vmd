...
namespace eval ::RMSXFlipbookTimeline::TimelineIO {
    ...
    proc attach_slice_targets {dataset folder} {
        ...
        for {set col 0} {$col < [llength $old_columns]} {incr col} {
            ...
            dict set column slice_number [expr {$col + 1}]
            if {$col < [llength $slice_files]} {
                set slice_file [file normalize [lindex $slice_files $col]]
                dict set column slice_file $slice_file
                if {[dict exists $molid_by_file $slice_file]} {
                    dict set column slice_molid [dict get $molid_by_file $slice_file]
                }
            }
    ...
    proc read_rmsx_csv {csv_path args} {
        ...
        if {$value_label eq ""} {
            set value_label [::RMSXFlipbookTimeline::NativeAnalysis::infer_metric_label $csv_path]
        }
        ...
        return [::RMSXFlipbookTimeline::Matrix::create \
            ...
            provenance [dict create file $csv_path]]
    }
    ...
    proc read_tml {filename args} {
        ...
        if {$free_selection} {
            return [read_free_selection $filename $headers $num_frames [dict get $opts molid] $lines]
        }
        return [read_residue_selection $filename $headers $num_frames [dict get $opts molid] $lines]
    }
