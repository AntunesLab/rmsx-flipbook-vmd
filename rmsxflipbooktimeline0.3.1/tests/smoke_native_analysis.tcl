...
proc assert_numeric_csv_parity {actual_path reference_path tolerance {metadata_columns {}}} {
    ...
    set actual_header [lindex $actual_rows 0]
    set reference_header [lindex $reference_rows 0]
    # The historical single-chain RMSF reference has no chain column. Compare
    # the same residue/value fields by header, preserving every numeric check.
    if {$actual_header eq {ResidueID ChainID RMSF} && $reference_header eq {ResidueID RMSF}} {
        set projected {}
        foreach row $actual_rows { lappend projected [list [lindex $row 0] [lindex $row 2]] }
        set actual_rows $projected
        set actual_header [lindex $actual_rows 0]
    }
    if {$actual_header ne $reference_header} {
...
set reference_csv [file join $workspace_root fixtures upstream test_files example1 chain_7_rmsx rmsx_mon_sys_0.015_ns.csv]
set reference_rmsd_csv [file join $workspace_root fixtures upstream test_files example1 chain_7_rmsx rmsd.csv]
set reference_rmsf_csv [file join $workspace_root fixtures upstream test_files example1 chain_7_rmsx rmsf.csv]
...
lassign [::TestHarness::legacy_numeric_rows [list $header_fields $row_fields]] header_fields row_fields
