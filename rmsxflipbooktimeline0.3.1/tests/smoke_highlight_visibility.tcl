...
::RMSXFlipbookTimeline::load_folder [file join $root fixtures seed_outputs reviewer-protease-9 combined] palette viridis view_preset rmsx
set ids [::RMSXFlipbookTimeline::state_get molids]
assert {[llength $ids] == 9} "Expected all nine slices"
set mid [lindex $ids 4]
set selector [atomselect $mid {chain B and resid 17}]
try {set residues [lsort -unique [$selector get residue]]} finally {$selector delete}
assert {[llength $residues] == 1} "B:17 is ambiguous"
...
try {
    ...
    # Locate the selected molecule in actual rendered pixels; a reset camera
    # need not put its center at exactly half the window width.
    foreach id $ids {if {$id != $mid} {mol off $id}}
    set target_bounds [::RMSXFlipbookTimeline::Render::render_to_tga [dict create method TachyonInternal background white] [file join $root outputs target-only.tga] 1600 720]
    ...
    foreach ns {::RMSXFlipbookTimeline::PlotWindow ::RMSXFlipbookTimeline::TimelinePlot} {
        ...
        set path [file join $root outputs [namespace tail $ns]-selected.tga]
        ::RMSXFlipbookTimeline::Render::render_to_tga [dict create method TachyonInternal background white] $path 1600 720
        set pixels [red_pixels $path]
        ...
        puts "[namespace tail $ns] selection pixels: $pixels"
