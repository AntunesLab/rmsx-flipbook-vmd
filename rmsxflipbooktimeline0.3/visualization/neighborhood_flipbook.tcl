...
namespace eval ::RMSXFlipbookTimeline::NeighborhoodFlipbook {
    ...
    variable snapshot_identity_maps {}
    ...
    proc restore_drawn_state {} {
        ...
        dict for {molid drawn} $drawn_state {
            ...
            if {[mol_drawn_state $molid] != 0} {continue}
            if {$drawn} {
                catch {mol on $molid}
            } else {
                catch {mol off $molid}
    ...
    proc focus_scene {} {
        ...
        catch {set top_state [molinfo top]}
    ...
    proc disable_geometry_modulation {} {
        foreach name {VMDMODULATERIBBON VMDMODULATENEWTUBE VMDMODULATENEWCARTOON} {
            ::RMSXFlipbookTimeline::Scene::unset_environment $name
        }
    }
    ...
    proc restore_modulation_env {} {
        ...
        ::RMSXFlipbookTimeline::Scene::release_environment [dict keys $modulation_env_state]
    ...
    proc focus_on_snapshots {molids} {
        ...
        foreach molid [molinfo list] {
            ...
            } elseif {[dict exists $drawn_state $molid]} {
                catch {mol off $molid}
            }
        }
        if {[llength $molids] > 0} {
    ...
    proc activate_mini_flipbook {molids files spacing offsets {value_result {}}} {
        ...
        # Opening a neighborhood does not alter the user's mouse bindings.
    }
    ...
    proc temp_pdb_path {} {
        set fp [file tempfile name rmsx_neighborhood_]
        close $fp
        return $name
    }

    proc row_identity_selection {row fallback {molid ""}} {
        variable snapshot_identity_maps
        if {![dict exists $row resid]} {
            return $fallback
        }
        if {[string trim [dict get $row resid]] eq ""} {
            return $fallback
        }
        if {$molid ne "" && [dict exists $snapshot_identity_maps $molid]} {
            set map [dict get $snapshot_identity_maps $molid]
            set actual [::RMSXFlipbookTimeline::ResidueIdentity::resolve $row [dict get $map records] [dict get $map source_molid]]
            return "residue [dict get $actual snapshot_residue]"
        }
        return [::RMSXFlipbookTimeline::ResidueIdentity::selection $row $molid]
    }

    proc install_snapshot_identity_map {source_molid frame selection snapshot_molid} {
        variable snapshot_identity_maps
        set source [atomselect $source_molid $selection frame $frame]
        try {
            set source_residues [$source get residue]
        } finally {$source delete}
        set target [atomselect $snapshot_molid all]
        try {set target_residues [$target get residue]} finally {$target delete}
        if {[llength $source_residues] != [llength $target_residues]} {
            error "Snapshot atom count changed; cannot preserve residue identity"
        }
        set mapping {}; set reverse {}
        foreach src $source_residues dst $target_residues {
            if {([dict exists $mapping $src] && [dict get $mapping $src] != $dst) ||
                ([dict exists $reverse $dst] && [dict get $reverse $dst] != $src)} {
                error "Snapshot merged or split source residues; cannot preserve residue identity"
            }
            dict set mapping $src $dst
            dict set reverse $dst $src
        }
        set source [atomselect $source_molid all]
        try {set records [::RMSXFlipbookTimeline::ResidueIdentity::from_selection $source]} finally {$source delete}
        set mapped {}
        foreach record $records {
            set residue [dict get $record residue]
            if {[dict exists $mapping $residue]} {
                dict set record snapshot_residue [dict get $mapping $residue]
                lappend mapped $record
            }
        }
        dict set snapshot_identity_maps $snapshot_molid [dict create source_molid $source_molid records $mapped]
        return 1
    }
    ...
    proc snapshot_selection {dataset source_molid frame selected_row_index} {
        ...
        if {$dataset ne {} && [dict exists $dataset rows]} {
            foreach row [dict get $dataset rows] {
                if {[dict exists $row atom_index] && ![dict exists $row resid]} {
                    lappend key_indices [dict get $row atom_index]
                    continue
                }
                set row_selection [row_identity_selection $row [::RMSXFlipbookTimeline::Matrix::row_selection $row] $source_molid]
        ...
        if {$dataset ne {} && [dict exists $dataset rows]} {
            ...
            if {$selected_row_index >= 0 && $selected_row_index < [llength $rows]} {
                ...
                set base [row_identity_selection $selected_row [::RMSXFlipbookTimeline::Matrix::row_selection $selected_row] $source_molid]
    ...
    proc assign_matrix_values {dataset molid column_index norm_column raw_column} {
        ...
        for {set row_index 0} {$row_index < [llength $rows]} {incr row_index} {
            ...
            set selection [row_identity_selection $row [::RMSXFlipbookTimeline::Matrix::row_selection $row] $molid]
            set thickness [::RMSXFlipbookTimeline::Values::clamp [expr {$user_scale * double($norm) + double($user_offset)}] 0.0 10.0]
            set ok 0
    ...
    proc category_selection_for_column {dataset column_index category_value {molid ""}} {
        set rows [dict get $dataset rows]
        set selections {}
        for {set row_index 0} {$row_index < [llength $rows]} {incr row_index} {
            set value [::RMSXFlipbookTimeline::Matrix::cell_value $dataset $row_index $column_index]
    ...
    proc apply_categorical_matrix_style {dataset molids column_indices} {
        ...
        foreach molid $molids column_index $column_indices {
            ...
            foreach category $categories {
                set selection [category_selection_for_column $dataset $column_index [dict get $category value] $molid]
                if {$selection eq ""} {
                    continue
                }
    ...
    proc clear {} {
        ...
        variable last_request_key
        restore_drawn_state
        if {[loaded_molid $top_state] && [lsearch -exact $snapshot_molids [molinfo top]] >= 0} {
            catch {mol top $top_state}
        }
        set top_state ""
    ...
    proc show_for_selection {dataset record args} {
        ...
        # Resolve before changing the scene, so ambiguous legacy rows fail safely.
        row_identity_selection $row $row_selection $source_molid
        ...
        if {[catch {
            ...
            foreach snapshot_molid $created_molids frame $frames {
                set row_highlight_selection [row_identity_selection $row $row_selection $snapshot_molid]
