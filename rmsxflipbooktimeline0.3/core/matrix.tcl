...
namespace eval ::RMSXFlipbookTimeline::Matrix {
    ...
    proc row_label {row} {
        ...
        if {[::RMSXFlipbookTimeline::ResidueIdentity::full $row]} {return [::RMSXFlipbookTimeline::ResidueIdentity::label $row]}
    ...
    proc row_selection {row} {
        if {[::RMSXFlipbookTimeline::ResidueIdentity::full $row]} {
            if {[dict exists $row source_molid] && [info commands molinfo] ne "" && [lsearch -exact [molinfo list] [dict get $row source_molid]] >= 0} {
                return [::RMSXFlipbookTimeline::ResidueIdentity::selection $row [dict get $row source_molid]]
            }
            return [::RMSXFlipbookTimeline::ResidueIdentity::portable_selection $row]
        }
        if {[dict exists $row selection] && [string trim [dict get $row selection]] ne ""} {return [dict get $row selection]}
        if {![dict exists $row resid]} {return ""}
        return [::RMSXFlipbookTimeline::ResidueIdentity::portable_selection $row]
    }
