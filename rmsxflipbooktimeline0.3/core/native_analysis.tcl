...
namespace eval ::RMSXFlipbookTimeline::NativeAnalysis {
    ...
    proc safe_chain_component {chain} {
        if {[regexp {^[A-Za-z0-9_-]+$} $chain]} {return $chain}
        return "id_[binary encode hex [encoding convertto utf-8 $chain]]"
    }
    proc track_working_molecule {molid} {
        variable working_scopes
        if {![llength $working_scopes]} {return}
        set scope [lindex $working_scopes end]
        dict lappend scope molids $molid
        lset working_scopes end $scope
        if {[dict get $scope cleanup]} {catch {mol off $molid}}
    }
    proc release_working_molecule {molid} {
        variable working_scopes
        set restore ""
        if {[llength $working_scopes] && [lsearch -exact [molinfo list] $molid] >= 0 && [molinfo top] == $molid} {
            set restore [dict get [lindex $working_scopes end] prior_top]
        }
        mol delete $molid
        if {$restore ne "" && [lsearch -exact [molinfo list] $restore] >= 0} {mol top $restore}
    }
    proc checkpoint {message} {
        variable working_scopes
        if {![llength $working_scopes]} {return}
        set callback [dict get [lindex $working_scopes end] callback]
        emit_progress [dict create progress_callback $callback] [dict create stage computing message $message]
    }
    proc relay_progress {callback stage target event} {
        set event [::RMSXFlipbookTimeline::OutputTxn::rewrite_paths $event $stage $target]
        if {[dict exists $event stage] && [dict get $event stage] eq "complete"} {
            dict set event stage computed
            dict set event message "Analysis computed; validating output"
        }
        if {$callback eq ""} {return ""}
        return [uplevel #0 [list {*}$callback $event]]
    }
    proc transactional_run {kernel topology trajectory output_dir args} {
        variable working_scopes
        ...
        set txn [::RMSXFlipbookTimeline::OutputTxn::begin $output_dir -overwrite $overwrite -inputs [list $topology $trajectory]]
        set stage [dict get $txn stage]
        set target [dict get $txn target]
        set relay [list ::RMSXFlipbookTimeline::NativeAnalysis::relay_progress $callback $stage $target]
        set prior_top ""
        if {[info commands molinfo] ne ""} {catch {set prior_top [molinfo top]}}
        lappend working_scopes [dict create callback $relay molids {} cleanup $cleanup prior_top $prior_top]
        set succeeded 0
        try {
            ...
            catch {set vmd_version [vmdinfo version]}
            set metadata [dict create plugin_version $::RMSXFlipbookTimeline::version vmd_version $vmd_version analysis $kernel topology [file normalize $topology] trajectory [file normalize $trajectory]]
            dict set metadata input_files [list [dict create path [file normalize $topology] size [file size $topology] mtime [file mtime $topology]] [dict create path [file normalize $trajectory] size [file size $trajectory] mtime [file mtime $trajectory]]]
            dict set metadata result [::RMSXFlipbookTimeline::OutputTxn::rewrite_paths $result $stage $target]
            ::RMSXFlipbookTimeline::OutputTxn::commit $txn $metadata
            set result [::RMSXFlipbookTimeline::OutputTxn::rewrite_paths $result $stage $target]
            dict set result run_id [dict get $txn id]
            if {[file exists [dict get $txn backup]]} {dict set result previous_output [dict get $txn backup]}
            dict set result published 1
            set succeeded 1
            # Publication is the commit point: a late UI observer failure cannot unpublish data.
            if {$callback ne ""} {
                catch {uplevel #0 [list {*}$callback [dict create stage complete output_dir $target message "Analysis output published"]]} notification
            }
            return $result
        } on error {message options} {
            catch {::RMSXFlipbookTimeline::OutputTxn::abort $txn}
            return -options $options $message
        } finally {
            set scope [lindex $working_scopes end]
            if {!$succeeded || $cleanup} {
                foreach molid [dict get $scope molids] {
                    if {[info commands molinfo] ne "" && [lsearch -exact [molinfo list] $molid] >= 0} {catch {release_working_molecule $molid}}
                }
            }
            set working_scopes [lrange $working_scopes 0 end-1]
        }
    }
    proc run {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_impl $topology $trajectory $output_dir {*}$args]
    }
    proc run_shift_map {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_shift_map_impl $topology $trajectory $output_dir {*}$args]
    }
    proc run_lddt_map {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_lddt_map_impl $topology $trajectory $output_dir {*}$args]
    }
    proc run_all_chains {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_all_chains_impl $topology $trajectory $output_dir {*}$args]
    }
    proc run_all_chain_shift_map {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_all_chain_shift_map_impl $topology $trajectory $output_dir {*}$args]
    }
    proc run_all_chain_lddt_map {topology trajectory output_dir args} {
        return [transactional_run [namespace current]::_run_all_chain_lddt_map_impl $topology $trajectory $output_dir {*}$args]
    }
    proc is_chain_group {group} {
        return [expr {![catch {dict get $group group_schema} schema] && $schema eq "1"}]
    }
    proc chain_label {chain} {
        if {[is_chain_group $chain]} {return [dict get $chain label]}
        return $chain
    }
    proc chain_group {field value {blank_segment 0}} {
        if {$field ni {chain segid}} {error "Unsupported chain group field: $field"}
        set label $value
        if {$value eq ""} {set label "(blank chain/segment)"}
        return [dict create group_schema 1 id [list $field $value $blank_segment] field $field value $value blank_segment $blank_segment label $label]
    }
    proc chain_group_selection {group} {
    ...
    proc resolve_chain_alias {molid alias} {
        set candidates {}; set index_sets {}
        foreach field {segid chain} {
            set group [chain_group $field $alias]
            set sel [atomselect $molid [chain_group_selection $group]]
            try {set indices [$sel get index]} finally {$sel delete}
            if {[llength $indices] && [lsearch -exact $index_sets $indices] < 0} {
                lappend candidates $group
                lappend index_sets $indices
            }
        }
        if {[llength $candidates] != 1} {
            error "Chain/segment name '$alias' identifies [llength $candidates] distinct atom groups; choose an exact discovered group or use the selection option"
        }
        return [lindex $candidates 0]
    }
    proc append_chain_selection {selection chain {molid ""}} {
        if {[string trim $chain] eq ""} {return $selection}
        if {![is_chain_group $chain]} {
            if {$molid eq ""} {error "Resolving a chain/segment name requires a target molecule"}
            set chain [resolve_chain_alias $molid $chain]
        }
        return "($selection) and ([chain_group_selection $chain])"
    }
    ...
    proc write_csv {filename residue_records slice_columns {format full}} {
        if {$format ni {full legacy}} {error "CSV identity format must be full or legacy"}
        if {$format eq "legacy" && ![::RMSXFlipbookTimeline::ResidueIdentity::legacy_unique $residue_records]} {
            error "Legacy CSV cannot represent these distinct residues; export full residue identity"
        }
        set header [expr {$format eq "full" ? [::RMSXFlipbookTimeline::ResidueIdentity::csv_header] : {ResidueID ChainID}}]
        foreach item $slice_columns {lappend header [lindex $item 0]}
        set rows {}
        foreach record $residue_records {
            if {[dict get $record index] % 128 == 0} {checkpoint "Preparing residue CSV output"}
            if {$format eq "full"} {set row [::RMSXFlipbookTimeline::ResidueIdentity::csv_fields $record]} else {set row [list [dict get $record resid] [dict get $record chain]]}
            foreach item $slice_columns {lappend row [lindex [lindex $item 1] [dict get $record index]]}
            lappend rows $row
        }
        write_simple_csv $filename $header $rows
    }
    ...
    proc write_rmsf_csv {filename residue_records values} {
        ...
        foreach record $residue_records value $values {
            lappend rows [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_fields $record] [list [numeric_csv_value $value]]]
        }
        write_simple_csv $filename [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_header] RMSF] $rows
    }

    proc residue_records_from_selection {selection} {
        return [::RMSXFlipbookTimeline::ResidueIdentity::from_selection $selection]
    }
    ...
    proc beta_values_for_selection_by_record_key {selection residue_records values} {
        set actual [residue_records_from_selection $selection]
        set by_residue {}
        foreach record $residue_records value $values {
            set resolved [::RMSXFlipbookTimeline::ResidueIdentity::resolve $record $actual [$selection molid]]
            dict set by_residue [dict get $resolved residue] $value
        }
        set out {}
        foreach residue [$selection get residue] {
            lappend out [expr {[dict exists $by_residue $residue] ? [dict get $by_residue $residue] : 0.0}]
        }
        return $out
    }
    ...
    proc record_key_from_parts {resid chain segid {insertion ""} {ordinal 0}} {
        return [list $chain $segid $resid $insertion $ordinal]
    }

    proc record_key {record} {
        return [::RMSXFlipbookTimeline::ResidueIdentity::key $record]
    }

    proc build_mask_metadata {molid full_selection residue_records mask_selection vmd_frame} {
        set masked_residues {}
        foreach clause [split [string trim $mask_selection] ";"] {
            if {[string trim $clause] eq ""} {continue}
            set vmd_clause [transform_mask_selection $clause]
            set sel [atomselect $molid "(($full_selection) and ($vmd_clause))" frame $vmd_frame]
            try {foreach residue [$sel get residue] {dict set masked_residues $residue 1}} finally {$sel delete}
        }
        set metadata {}
        foreach record $residue_records {
            dict set record masked [dict exists $masked_residues [dict get $record residue]]
            lappend metadata $record
        }
        return $metadata
    }
    ...
    proc write_mask_metadata {filename metadata} {
        ...
        foreach record $metadata {
            lappend rows [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_fields $record] [list [expr {[dict get $record masked] ? "True" : "False"}]]]
        }
        write_simple_csv $filename [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_header] Masked] $rows
    }
        ...
        for {set row_index 0} {$row_index < [llength $residue_records]} {incr row_index} {
            ...
            foreach item $slice_columns {
                ...
                lappend entries [list \
                    ...
                    $time_slice \
                    [::RMSXFlipbookTimeline::ResidueIdentity::portable $record]]
            }
        }
    ...
    proc write_summary_csv {filename summary} {
        ...
        foreach group {top bottom} {
            ...
            foreach entry [dict get $summary $group] {
                incr rank
                set record [lindex $entry 4]
                if {$record eq ""} {set record [dict create resid [lindex $entry 1] chain [lindex $entry 2]]}
                lappend rows [concat [list $group $rank] [::RMSXFlipbookTimeline::ResidueIdentity::csv_fields $record] [list [lindex $entry 3] [numeric_csv_value [lindex $entry 0]]]]
            }
        }
        write_simple_csv $filename [concat {Group Rank} [::RMSXFlipbookTimeline::ResidueIdentity::csv_header] {TimeSlice RMSX}] $rows
    }
    ...
    proc pairwise_distances {coords} {
        ...
        for {set i 0} {$i < $count} {incr i} {
            if {$i % 32 == 0} {checkpoint "Computing residue distances"}
            set row {}
            set a [lindex $coords $i]
            for {set j 0} {$j < $count} {incr j} {
                if {$j > 0 && $j % 4096 == 0} {checkpoint "Computing residue distances"}
    ...
    proc lddt_instability_values {reference_model current_coords thresholds empty_neighbor_instability} {
        ...
        for {set i 0} {$i < [llength $current_coords]} {incr i} {
            if {$i % 32 == 0} {checkpoint "Scoring local distance preservation"}
    ...
    proc load_structure_file {path requested_type} {
        ...
        if {[molinfo $molid get numatoms] == 0} {
            catch {release_working_molecule $molid}
            error "Topology contains no atoms: $normalized"
        }
    ...
    proc load_trajectory {topology trajectory args} {
        ...
        try {
            ...
            checkpoint "Trajectory coordinates loaded"
            set total_frames [molinfo $molid get numframes]
            return [dict create molid $molid topology_type [dict get $top_info type] trajectory_type $trajectory_type initial_frame_count $initial_frames total_frame_count $total_frames]
        } on error {message options} {
            catch {release_working_molecule $molid}
            return -options $options $message
        }
    ...
    proc emit_progress {opts event} {
        ...
        if {$lowered in {cancel cancelled stop abort}} {
            return -code error -errorcode {RMSXFLIPBOOK CANCELLED} "Native analysis cancelled by progress callback"
        }
        return $result
    }
    ...
    proc path_is_within {parent child} {return [::RMSXFlipbookTimeline::OutputTxn::within $parent $child]}
    ...
    proc is_rmsx_managed_output_dir {output_dir} {
        if {![file exists $output_dir]} {return 1}
        if {![file isdirectory $output_dir]} {return 0}
        if {![llength [::RMSXFlipbookTimeline::OutputTxn::entries $output_dir]]} {return 1}
        return [expr {![catch {::RMSXFlipbookTimeline::OutputTxn::verify_owned $output_dir}]}]
    }
    ...
    proc prepare_managed_output_dir {output_dir args} {
        set opts [::RMSXFlipbookTimeline::parse_kv_options [dict create overwrite 0 verbose 1 topology_file "" trajectory_file ""] {*}$args]
        set output_dir [file normalize $output_dir]
        if {[llength [::RMSXFlipbookTimeline::OutputTxn::entries $output_dir]]} {
            error "Output already contains files; use transactional analysis to replace a verified result"
        }
        file mkdir $output_dir
        return [dict create output_dir $output_dir sentinel ""]
    }

    proc clean_previous_slice_outputs {output_dir} {
        error "In-place slice cleanup is disabled; publish a new result through OutputTxn"
    }
    ...
    proc chain_groups_for_molecule {molid analysis_selection} {
        set sel [atomselect $molid $analysis_selection frame 0]
        try {set atoms [$sel get {chain segid}]} finally {$sel delete}
        set groups {}
        foreach atom $atoms {
            lassign $atom chain segid
            foreach field {chain segid} {if {[string trim [set $field]] eq ""} {set $field ""}}
            if {$segid ne ""} {set group [chain_group segid $segid]} else {set group [chain_group chain $chain 1]}
            dict set groups [dict get $group id] $group
        }
        set ordered {}
        foreach id [lsort -dictionary [dict keys $groups]] {lappend ordered [dict get $groups $id]}
        set labels [lmap group $ordered {dict get $group label}]
        set result {}
        foreach group $ordered {
            set label [dict get $group label]
            if {[llength [lsearch -all -exact $labels $label]] > 1} {
                dict set group label "[dict get $group field] $label"
            }
            lappend result $group
        }
        return $result
    }
    proc discover_chain_groups {topology analysis_selection {topology_type auto}} {
        set prior_top ""
        ...
        try {
            return [chain_groups_for_molecule $molid $analysis_selection]
        } finally {
            set owned_top [expr {[lsearch -exact [molinfo list] $molid] >= 0 && [molinfo top] == $molid}]
            catch {release_working_molecule $molid}
            if {$owned_top && $prior_top ne "" && [lsearch -exact [molinfo list] $prior_top] >= 0} {catch {mol top $prior_top}}
        }
    }
    proc discover_valid_chains {topology analysis_selection {topology_type auto}} {
        return [lmap group [discover_chain_groups $topology $analysis_selection $topology_type] {dict get $group label}]
    }

    proc clean_previous_combined_outputs {output_dir} {
        error "In-place combined-output cleanup is disabled; publish a new result through OutputTxn"
    }
    ...
    proc combine_rmsf_csvs {chain_dirs combined_dir args} {
        set opts [::RMSXFlipbookTimeline::parse_kv_options [dict create output_name rmsf.csv overwrite 0] {*}$args]
        set rows {}
        foreach dir $chain_dirs {
            set path [file join $dir rmsf.csv]
            if {![file exists $path]} {continue}
            set data [read_simple_csv $path]
            set header [dict get $data header]
            set vi [csv_column_index $header RMSF]
            foreach row [dict get $data rows] {
                set record [::RMSXFlipbookTimeline::ResidueIdentity::csv_record $header $row [llength $rows]]
                if {![dict exists $record chain]} {dict set record chain [chain_id_from_output_dir $dir]}
                lappend rows [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_fields $record] [list [lindex $row $vi]]]
            }
        }
        if {![llength $rows]} {return ""}
        set path [file join $combined_dir [dict get $opts output_name]]
        if {[file exists $path] && ![dict get $opts overwrite]} {error "Combined RMSF CSV already exists: $path"}
        write_simple_csv $path [concat [::RMSXFlipbookTimeline::ResidueIdentity::csv_header] RMSF] $rows
        return $path
    }
    ...
    proc read_mask_metadata_file {filename} {
        ...
        set data [read_simple_csv $filename]
        set header [dict get $data header]
        set mask_index [csv_column_index $header Masked]
        set out {}
        foreach row [dict get $data rows] {
            set item [::RMSXFlipbookTimeline::ResidueIdentity::csv_record $header $row [llength $out]]
            dict set item masked [expr {[string tolower [string trim [lindex $row $mask_index]]] in {1 true yes masked}}]
            lappend out $item
        }
        return $out
    }
    proc align_mask_metadata {records masks} {
        if {![llength $masks]} {return {}}
        set assigned {}
        foreach mask $masks {
            set record [::RMSXFlipbookTimeline::ResidueIdentity::resolve $mask $records]
            set index [dict get $record index]
            if {[dict exists $assigned $index]} {error "Duplicate mask identity: [residue_record_label $record]"}
            dict set assigned $index [dict get $mask masked]
        }
        set aligned {}
        foreach record $records {
            set index [dict get $record index]
            dict set record masked [expr {[dict exists $assigned $index] ? [dict get $assigned $index] : 0}]
            lappend aligned $record
        }
        return $aligned
    }
    ...
    proc read_simple_csv {filename} {
        ...
        for {set i 0} {$i < [string length $text]} {incr i} {
            ...
            if {$quoted} {
                if {$c eq "\""} {
                    if {[string index $text [expr {$i+1}]] eq "\""} {append field $c; incr i} else {set quoted 0}
                } else {append field $c}
            } elseif {$c eq "\"" && $field eq ""} {set quoted 1
            } elseif {$c eq ","} {lappend record $field; set field ""
            } elseif {$c eq "\n" || $c eq "\r"} {
                if {$c eq "\r" && [string index $text [expr {$i+1}]] eq "\n"} {incr i}
                if {$field ne "" || [llength $record]} {lappend record $field; lappend records $record}
                set field ""; set record {}
            } else {append field $c}
        }
        if {$quoted} {error "Unterminated CSV quote: $filename"}
        if {$field ne "" || [llength $record]} {lappend record $field; lappend records $record}
        if {![llength $records]} {error "CSV file is empty: $filename"}
        return [dict create header [lindex $records 0] rows [lrange $records 1 end]]
    }
    ...
    proc residue_record_label {record} {return [::RMSXFlipbookTimeline::ResidueIdentity::label $record]}

    proc csv_to_records_and_columns {csv_data} {
        set header [dict get $csv_data header]
        if {[lsearch -exact $header ResidueID] < 0} {error "CSV does not contain ResidueID"}
        set slice_indices {}; set slice_columns {}
        for {set i 0} {$i < [llength $header]} {incr i} {
            set name [lindex $header $i]
            if {[matrix_slice_header $name]} {lappend slice_indices $i; lappend slice_columns [list $name {}]
            } elseif {$name ni {ResidueID ChainID SegID InsertionCode ResidueOrdinal}} {error "Unexpected matrix CSV column: $name"}
        }
        if {![llength $slice_indices]} {error "CSV has no slice columns"}
        set records {}; set seen {}
        foreach row [dict get $csv_data rows] {
            if {[llength $row] != [llength $header]} {error "CSV row width does not match header"}
            set record [::RMSXFlipbookTimeline::ResidueIdentity::csv_record $header $row [llength $records]]
            set key [record_key $record]
            if {[dict exists $seen $key]} {error "Ambiguous duplicate residue in matrix CSV: [residue_record_label $record]"}
            dict set seen $key 1
            lappend records $record
            set ci 0
            foreach source_index $slice_indices {
                set value [string trim [lindex $row $source_index]]
                if {![string is double -strict $value]} {error "Non-numeric matrix value: $value"}
                set item [lindex $slice_columns $ci]
                lset item 1 [concat [lindex $item 1] [list $value]]
                lset slice_columns $ci $item
                incr ci
            }
        }
        return [dict create residue_records $records slice_columns $slice_columns slice_indices $slice_indices]
    }

    proc compute_global_csv_value_range {csv_paths} {
        set values {}
        foreach path $csv_paths {
            set parsed [csv_to_records_and_columns [read_simple_csv $path]]
            set masks [align_mask_metadata [dict get $parsed residue_records] [read_mask_metadata_file [file join [file dirname $path] masked_residues.csv]]]
            foreach column [dict get $parsed slice_columns] {
                set i 0
                foreach value [lindex $column 1] {
                    if {$i >= [llength $masks] || ![dict get [lindex $masks $i] masked]} {lappend values $value}
                    incr i
                }
            }
        }
        if {![llength $values]} {error "No unmasked RMSX values found"}
    ...
    proc clip_csv_masked_rows {csv_path clip_bounds decimals} {
        set data [read_simple_csv $csv_path]
        set parsed [csv_to_records_and_columns $data]
        set masks [align_mask_metadata [dict get $parsed residue_records] [read_mask_metadata_file [file join [file dirname $csv_path] masked_residues.csv]]]
        lassign $clip_bounds lo hi
        set out {}; set ri 0
        foreach row [dict get $data rows] {
            if {$ri < [llength $masks] && [dict get [lindex $masks $ri] masked]} {
                foreach col [dict get $parsed slice_indices] {lset row $col [format "%.${decimals}f" [expr {min($hi,max($lo,double([lindex $row $col])))}]]}
            }
            lappend out $row; incr ri
        }
        write_simple_csv $csv_path [dict get $data header] $out
        return [dict create header [dict get $data header] rows $out mask_metadata $masks]
    }

    proc write_summary_from_csv {csv_path summary_path summary_n} {
        set csv_data [read_simple_csv $csv_path]
        set parsed [csv_to_records_and_columns $csv_data]
        set mask_metadata [align_mask_metadata [dict get $parsed residue_records] [read_mask_metadata_file [file join [file dirname $csv_path] masked_residues.csv]]]
    ...
    proc method_details {kernel result} {
        ...
        dict set method transform none
        if {[dict exists $result log_transform] && [::RMSXFlipbookTimeline::truthy [dict get $result log_transform]]} {
            dict set method transform ln1p
            dict set method input_unit [dict get $method unit]
            dict set method unit unitless
    ...
    proc csv_method_metadata {csv_path} {
        set csv_path [file normalize $csv_path]
        set inferred [dict create value_label [legacy_metric_label $csv_path] unit unknown transform unknown metadata_source filename]
        foreach directory [list [file dirname $csv_path] [file dirname [file dirname $csv_path]]] {
            set path [file join $directory .rmsx_output_manifest.tcldict]
            if {![file isfile $path]} {continue}
            if {[catch {
                set metadata [::RMSXFlipbookTimeline::OutputTxn::read_dict $path]
                if {[dict get $metadata schema] != 2 || [dict get $metadata owner] ne "rmsxflipbooktimeline" || ![dict get $metadata complete]} {error "Unrecognized metadata"}
                set relative [string range $csv_path [expr {[string length $directory]+1}] end]
                if {[dict get $metadata files $relative] ne [::RMSXFlipbookTimeline::OutputTxn::digest $csv_path]} {error "CSV differs from its provenance"}
                set result [dict get $metadata result]
                if {[dict exists $result method]} {set method [dict get $result method]} else {set method [method_details [dict get $metadata analysis] $result]}
                dict set method metadata_source $path
                dict set method run_id [dict get $metadata run_id]
            }]} {continue}
            return $method
        }
        return $inferred
    }
    proc infer_metric_label {csv_path} {return [dict get [csv_method_metadata $csv_path] value_label]}
    proc legacy_metric_label {csv_path} {
    ...
    proc read_rmsf_points_for_records {csv_path residue_records} {
        ...
        set source {}
        foreach row [dict get $data rows] {
            set record [::RMSXFlipbookTimeline::ResidueIdentity::csv_record $header $row [llength $source]]
            dict set record value [lindex $row $vi]
            lappend source $record
        }
        set out {}; set i 0
        foreach record $residue_records {
            set matches {}
            foreach candidate $source {
                if {[::RMSXFlipbookTimeline::ResidueIdentity::matches $candidate $record]} {lappend matches $candidate}
            }
            if {[llength $matches] > 1} {error "Ambiguous RMSF residue: [residue_record_label $record]"}
            if {[llength $matches] == 1} {lappend out [list $i [dict get [lindex $matches 0] value]]}
            incr i
        }
        return $out
    }
    ...
    proc export_svg_atomic {kind folder args} {
        set output_name [expr {$kind eq "heatmap" ? "rmsx_heatmap.svg" : "rmsx_report.svg"}]
        foreach {key value} $args {if {[string trimleft $key -] eq "output_name"} {set output_name $value}}
        if {[file pathtype $output_name] eq "relative"} {set output_name [file join $folder $output_name]}
        return [::RMSXFlipbookTimeline::OutputTxn::atomic_write $output_name [list [namespace current]::export_svg_body $kind $folder $args]]
    }
    proc export_svg_body {kind folder args_list temporary_path} {
        return [_write_${kind}_svg_impl $folder {*}$args_list -output_name $temporary_path]
    }
    proc write_heatmap_svg {folder args} {return [export_svg_atomic heatmap $folder {*}$args]}
    proc write_report_svg {folder args} {return [export_svg_atomic report $folder {*}$args]}

    proc _write_heatmap_svg_impl {folder args} {
        ...
        set residue_records [dict get $parsed residue_records]
        set slice_columns [dict get $parsed slice_columns]
        set mask_metadata [align_mask_metadata $residue_records [read_mask_metadata_file [file join $output_dir masked_residues.csv]]]
    ...
    proc _write_report_svg_impl {folder args} {
        set defaults [dict create \
            csv "" \
            rmsd_csv rmsd.csv \
            rmsf_csv rmsf.csv \
    ...
    proc whole_trajectory_rmsf {molid analysis_selection vmd_start vmd_end} {
        ...
        try {
            ...
            checkpoint "Computing whole-trajectory RMSF"
            set values [measure rmsfperresidue $sel first $vmd_start last $vmd_end step 1]
            checkpoint "Whole-trajectory RMSF computed"
            return $values
        } finally {
            catch {$sel delete}
        }
    ...
    proc fitted_rmsd_rows {molid analysis_selection vmd_start vmd_end traj_start time_step time_origin} {
        ...
        try {
            for {set vmd_frame $vmd_start} {$vmd_frame <= $vmd_end} {incr vmd_frame} {
                ...
                checkpoint "Computing fitted RMSD"
                ...
                try {
                    ...
                    } finally {$sel set {x y z} $original_coords}
    ...
    proc _run_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            chain "" \
        ...
        emit_progress $opts [dict create \
            ...
            chain [chain_label [dict get $opts chain]] \
        ...
        set full_selection [append_chain_selection [dict get $opts selection] [dict get $opts chain] $molid]
        set analysis_selection [append_chain_selection [dict get $opts analysis_selection] [dict get $opts chain] $molid]
        emit_progress $opts [dict create \
            metric rmsx \
            stage started \
            output_dir $output_dir \
        ...
        dict set result parameters [dict remove $opts progress_callback overwrite cleanup verbose]
        return $result
    }

    proc _run_shift_map_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            chain "" \
            analysis_type protein \
            full_backbone 1 \
    ...
    proc _run_lddt_map_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            chain "" \
            analysis_type protein \
            full_backbone 1 \
    ...
    proc _run_all_chains_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            analysis_type protein \
            full_backbone 1 \
            selection "" \
        ...
        set chain_groups [discover_chain_groups \
            [file normalize $topology] \
            [dict get $opts analysis_selection] \
            [dict get $opts topology_type]]
        set chains [lmap group $chain_groups {dict get $group label}]
        if {[llength $chains] == 0} {
        ...
        set chain_index 0
        foreach group $chain_groups {
            set chain [dict get $group label]
            incr chain_index
            set chain_dir [file join $output_dir "chain_[safe_chain_component $chain]_rmsx"]
            if {[dict get $opts verbose]} {
    ...
    proc _run_all_chain_shift_map_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            analysis_type protein \
            full_backbone 1 \
            selection "" \
        ...
        foreach group $chain_groups {
            set chain [dict get $group label]
            incr chain_index
            set chain_dir [file join $output_dir "chain_[safe_chain_component $chain]_shiftmap"]
            if {[dict get $opts verbose]} {
    ...
    proc _run_all_chain_lddt_map_impl {topology trajectory output_dir args} {
        set defaults [dict create \
            analysis_type protein \
            full_backbone 1 \
            selection "" \
        ...
        foreach group $chain_groups {
            set chain [dict get $group label]
            incr chain_index
            set chain_dir [file join $output_dir "chain_[safe_chain_component $chain]_lddtmap"]
            if {[dict get $opts verbose]} {
